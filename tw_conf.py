...
# ---- 目标空间硬门（2026-09-05 补漏③：空间从「仅展示」改为「不足即剔除」；09-06 曾收紧为 6×ATR/AND，09-07 回退：因 6×1hATR 门槛过严致 9/6 起零推送）----
MIN_SPACE_PCT = 3.0     # 目标空间（现价→目标位）>= 3% 才算有操作价值
MIN_SPACE_ATR = 1.5     # 目标空间 >= 1.5×ATR(1h) 才算盈亏比合格（与 PCT 任一满足即通过，OR）
